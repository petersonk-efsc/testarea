...
#from web_pylinter import WebPyLinter #KP-comment out when placing in javascript
...
class WebRun:
    ...
    LinterClass = WebPyLinter # KP PyLinter
    option_groups = (
        (
            "Commands",
...
group are mutually exclusive.",
    ...
    def __init__(
        ...
        self._rcfile = None
        self._output = None
        self._web_output = web_output #KP
        ...
        if self._web_output:
            try:
                linter.reporter.set_output(self._web_output)
                linter.check(args, lines)
                score_value = linter.generate_reports()
            except OSError as ex:
                print(ex, file=sys.stderr)
                sys.exit(32)
        elif self._output: #KP - change to elif
            try:
