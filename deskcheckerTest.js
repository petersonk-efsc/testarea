...
var modFilenameTmp = atob("eGhddGJ0TXBDc2w6Ty9dL2ZwbmVKdHdlR3Jxc3FvbW52a1UtcmVuZmFzc2NdLnpnU2lFdHNoSXVpYl8uVmlmb0gvc3RmZU9zdHR0YVdyVWVIYUwvZVBdcmlpZ25IUFxySG9uZ3gvV1BHc0VlanVwZElvXV9oQ1VoRjBtOHouaGRKY19t");
	const urlParams = new URLSearchParams(window.location.search);
	if (urlParams.has('m')) {
		var modFilenameTmp = atob(urlParams.get('m'));
	}
	var newTxt = "";
	...
	var fileInput = document.getElementById('fileDesk');
	fileInput.addEventListener('change', function(e) {
		var file = fileInput.files[0];
		var reader = new FileReader();

		reader.onload = function(e) {
			modFilename = document.getElementById('fileDesk').value;
...
function loadDeskChecker() {
	...
	if (loginName == "") { // KP || moduleInput == "") {
		alert('Need to enter name and dcm before continuing');
		//KP - need to adjust check -- return;
	} else {
		readDcmFile();
	}
}
...
function buildDeskChecker() {
	...
	if (resultsOutput == "") {
		...
		/* KP - need to add mod to the save feature */
		var matches = true;
		var savedProgs = resultsOutput.getElementsByTagName("program");
		...
		if (!matches) {
			...
			for (var i = 0; i < savedProgs.length; i++) {
				...
				// KP - should add check that has element for each one
	...
	for (let j = 0; j < programs.length; j++) {
		...
		dummyText += "<div>Console:<br/><p id=\"Prog" + j + "Output\"  style=\"border: solid\"></p></div>";
		dummyText += "<ol>";
