...
class WebPyLinter(
    ...
    * handle some basic but necessary stats'data (number of classes, methods...)
    ...
    def __init__(self, options=(), reporter=None, option_groups=(), pylintrc=None):
        ...
        self.options = options + WebPyLinter.make_options() #KP
        self.option_groups = option_groups + WebPyLinter.option_groups #KP
    ...
    def check(self, files_or_modules, lines=None): #KP - added lines=None
        """main checking entry: check a list of files or modules from their name.

        files_or_modules is either a string or list of strings presenting modules to check.
        """

        print('KP - in WebPyLinter check')
        self.initialize()
        ...
        if self.config.from_stdin:
            ...
            with fix_import_path(files_or_modules):
                if lines:
                    ...
                    the_data = _read_stdin()
                self._check_files(
                    functools.partial(self.get_ast, data=the_data), # KP data=_read_stdin()),
